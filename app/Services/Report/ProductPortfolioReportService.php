...
class ProductPortfolioReportService
{
    public function __construct(
        private DtcSalesQueryService $dtcQuery,
        private OdooB2bSalesService $b2bService,
    ) {}

    /**
     * Generate the full data array for the Product Portfolio Overview PDF.
     *
     * @return array<string, mixed>
     */
    public function generate(string $since): array
    {
        $dtcByProduct = $this->dtcQuery->productSalesDetailed($since);
        $dtcByCategory = $this->dtcQuery->categorySales($since, now()->toDateString());
        $products = $this->dtcQuery->productCatalog();
        $stock = $this->dtcQuery->stockData();
        $b2bByProduct = $this->b2bService->salesByProduct($since);
        ...
        return [
            ...
            'intro' => 'Overzicht van alle Cyclowax producten met verkoopcijfers uit Shopify (DTC) en Odoo (B2B). '
                .'Scope: '.$since.' t/m heden. DTC data uit dashboard database, B2B data uit Odoo sale.order.',
            'metrics' => [
            ...
            'sections' => $this->buildSections($dtcByProduct, $dtcByCategory, $products, $stock, $b2bByProduct, $since),
        ];
    }
    ...
    private function buildSections(
        ...
        $sections[] = $this->buildDiscontinuedTable($products);
        $sections[] = $this->buildLongTailTable($dtcByProduct, $b2bByProduct);

        // ── Data Completeness ──
        $sections[] = ['type' => 'page-break'];
        $sections[] = ['type' => 'heading', 'content' => 'Data Completeness & Notes'];
        $sections[] = $this->buildDataCompletenessSection($dtcByProduct, $b2bByProduct, $products, $stock, $since);
    ...
    private function buildDiscontinuedTable(array $products): array
    {
    ...
    private function buildDataCompletenessSection(array $dtcByProduct, array $b2bByProduct, array $products, array $stock, string $since): array
    {
        ...
        $items = [
            ...
            '<strong>Scope:</strong> '.$since.' t/m heden. DTC excl. voided en refunded orders.',
        ];
